...
// Randomly place mines field
private _numtrap = ceil ((random [20, 40, 80]) + (_enemyStrength * 10));
private _mineFieldArea = (_enemyStrength * 10);
private _mineField = [_missionArea, 0, _aoSize, 20, 0, 0.1, 0, _blackList] call BIS_fnc_findSafePos;
for "_i" from 1 to _numtrap do
{
    private _trapPos = [_mineField, 0, 50, 0, 0, 0.1, 0, _blackList] call BIS_fnc_findSafePos;
...
_blackList append [[_mineField, _mineFieldArea]];
...
for "_i" from 1 to _numAA do
{
    if (count units east < _AIlimit) then {
        ...
        if (count _spawnPos == 2) then {
            ...
            if (_aaClass == "O_Radar_System_02_F") then {
                for "_i" from 1 to (ceil _enemyStrength / 3) do {
                    ...
                    private _aaUnit = createVehicle ["O_SAM_System_04_F", _spawnPos, [], 0, "NONE"];
                    _aaUnit setDir random 360;
                    createVehicleCrew _aaUnit;
                };
            };
            if (selectRandomWeighted [true, 0.75, false, 1] == true && _aaClass != "vn_o_static_rsna75") then {
                [_spawnPos, 0, SelectRandom OPFORcamps, 0.1] call BIS_fnc_objectsMapper;
            };
...
// Spawn air units
private _numAir = floor (random (_enemyStrength / 5) + (_minEnemy / 6)) min 3;
...
for "_i" from 1 to _numPatrols do
{
    if (count units east < _AIlimit) then {
        ...
        private _spawnPos = [_missionArea, 0, _aoSize, 5, 0, 0.3, 0, _blackList] call BIS_fnc_findSafePos;
        if (count _spawnPos == 2) then {
            private _patrolGroup = [_spawnPos, EAST, _array_infantry, [], [], [], [], [2,0.5], 0, false, _enemyStrength] call BIS_fnc_spawnGroup;	
...
for "_i" from 1 to _numSquadUnits do
{
    if (count units east < _AIlimit) then {
        ...
        private _spawnPos = [_missionArea, 0, _aoSize, 5, 0, 0.3, 0, _blackList] call BIS_fnc_findSafePos;
        if (count _spawnPos == 2) then {
             private _SquadUnit = [_spawnPos, EAST, _array_infantry, [], [], [], [], [4,0.8]] call BIS_fnc_spawnGroup;
            [_SquadUnit, _spawnPos] call BIS_fnc_taskDefend;
            sleep 0.1;   
        };
    } else {OPFORreserves = OPFORreserves + 1; publicVariable "OPFORreserves"};
